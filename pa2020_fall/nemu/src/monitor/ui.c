...
uint32_t vaddr_read(paddr_t paddr, int sreg, size_t len);
...
cmd_handler(cmd_x) {
    ...
    for(int i = 0; i < n; ++i) {
        uint32_t val = vaddr_read(addr + 4*i, SREG_CS, 4);
        printf("0x%08x\t", val);
    }
    printf("\n");
    return 0;
...
static struct
{
	...
	{"q", "Exit NEMU", cmd_q},
	{"p", "Evaluate an expression in hex", cmd_p},
	...
	{"pd", "Evaluate an expression in dec", cmd_pd},
    {"x", "Scan specific position in hex in nemu memory", cmd_x}
